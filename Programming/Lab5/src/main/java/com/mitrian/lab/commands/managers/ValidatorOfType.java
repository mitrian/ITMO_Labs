...
public class ValidatorOfType {
    ...
    public static Integer validationInteger( Set<ExtraCheck> dop) throws ForcedShutdownException {
            try {
                if (!scanner.hasNextLine()) {
                    throw new ForcedShutdownException();
                }
                ...
                if ("".equals(localScanner)){
                    if (dop.contains(ExtraCheck.NULL)){
                        return null;
                    }
                    throw new NumberFormatException();

                }
                int type = Integer.parseInt(localScanner);
                if (dop.contains(ExtraCheck.LESS885) && type>884){
                    throw new NumberFormatException();
                }
                if (dop.contains(ExtraCheck.MORE0) && type<=0){
                    throw new NumberFormatException();
                }
...
//            }
            ...
            try {
                ...
                //ZonedDateTime.of(LocalDateTime.parse(scanner.nextLine(),formatter),ZoneId.systemDefault());
    ...
    public static Status validationStatus() throws ForcedShutdownException {
            String nullChecker;
    ...
    public static Country validationCountry() throws ForcedShutdownException {
