...
public class UDPChannelServer implements AutoCloseable {
	...
	public UDPChannelServer(
		...
		try {
			...
			LOGGER.info("Сервер успешно запущен на следующем порте " + address.getPort());
		}
		catch (IOException e) {
			throw new NetworkException("Ошибка при открытии DatagramChannel или Selector", e);
		}
...
//				Change gotAll state if got the last UDPFrame
			...
			throw new NetworkException("Ошибка при мапинге запроса", e);
		}
		catch (IOException e) {
			throw new NetworkException("Ошибка запроса с сервера", e);
		}
	}
	 ...
	 *
	 * @param abstractResponse response instance
	 ...
	 */
	public void sendResponse(AbstractResponse abstractResponse) throws NetworkException {
//		Throwing NPE if response is null
		Objects.requireNonNull(abstractResponse);
...
//			Check if response should be divided into separate chunks
			if (responseBytes.length > NetworkUtils.RESPONSE_BUFFER_SIZE)
				...
				sendResponseNoOverhead(responseBytes, abstractResponse.getTo());
		}
		catch (MappingException e) {
			throw new NetworkException("Ошибка при преобразовании response в байты", e);
		}
	}
...
//		Sending all response frames to the client
		try {
			...
			throw new NetworkException("Ошибка при получение response with overhead от сервера ", e);
		}
	}
...
//			Sending response frame to the client
			...
			throw new NetworkException("Ошибка при получение response without overhead от сервера ", e);
		}
	}
	...
	public void close() throws NetworkException {
		try {
			...
			throw new NetworkException("Невозможно закрыть Selector или DatagramChannel", e);
		}
	}
